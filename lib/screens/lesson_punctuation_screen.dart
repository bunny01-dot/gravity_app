...
// SCREEN: Punctuation
// -----------------------------------------------------------------------------
class LessonPunctuationScreen extends StatefulWidget {
  const LessonPunctuationScreen({super.key});

  @override
  State<LessonPunctuationScreen> createState() =>
      _LessonPunctuationScreenState();
}

class _LessonPunctuationScreenState extends State<LessonPunctuationScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_Punctuation/';
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_punctuation') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_punctuation', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_punctuation_story_completed') ?? false;
    bool quizDone = prefs.getBool('lesson_punctuation_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_punctuation_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_punctuation_quiz_completed', true);
        await prefs.setBool('lesson_punctuation_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_punctuation_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Punctuation!\nGreat job!"
