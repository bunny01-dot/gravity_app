...
// -----------------------------------------------------------------------------
// SCREEN: Correlative Conjunctions
// -----------------------------------------------------------------------------
class LessonCorrelativeConjunctionsScreen extends StatefulWidget {
  const LessonCorrelativeConjunctionsScreen({super.key});

  @override
  State<LessonCorrelativeConjunctionsScreen> createState() =>
      _LessonCorrelativeConjunctionsScreenState();
}

class _LessonCorrelativeConjunctionsScreenState
    extends State<LessonCorrelativeConjunctionsScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_Correlative_Conjunctions/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('correlative_conjunctions');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_correlative') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_correlative', true);
  ...
  void _initializeLessonContent() {
    _slides = [
      ...
      LessonQuizInteraction(
        ...
        correctIndex: 1,
        explanation:
            "Correct! 'Girls' is closer to the verb, so use plural 'are'.",
        imagePath: 'either_or_square.webp',
      ),
      // 6. Neither...Nor
      LessonSlide(
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_correlative_story_completed') ?? false;
    bool quizDone = prefs.getBool('lesson_correlative_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_correlative_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_correlative_quiz_completed', true);
        await prefs.setBool('lesson_correlative_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_correlative_score', score);
        }
      }
        ...
        Expanded(
          ...
          child: Container(
            ...
            child: ClipRRect(
              ...
              child: LessonImage(
                lessonId: 'correlative_conjunctions',
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Correlative Conjunctions!\nGreat job!"
