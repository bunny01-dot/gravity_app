...
// SCREEN: Conditionals
// -----------------------------------------------------------------------------
class LessonConditionalsScreen extends StatefulWidget {
  const LessonConditionalsScreen({super.key});

  @override
  State<LessonConditionalsScreen> createState() =>
      _LessonConditionalsScreenState();
}

class _LessonConditionalsScreenState extends State<LessonConditionalsScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_Conditionals/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('conditionals');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_conditionals') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_conditionals', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_conditionals_story_completed') ?? false;
    bool quizDone =
        prefs.getBool('lesson_conditionals_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_conditionals_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_conditionals_quiz_completed', true);
        await prefs.setBool('lesson_conditionals_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_conditionals_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Conditionals!\nGreat job!"
