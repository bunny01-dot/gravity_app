...
// -----------------------------------------------------------------------------
class LessonLinkingWordsScreen extends StatefulWidget {
  const LessonLinkingWordsScreen({super.key});

  @override
  State<LessonLinkingWordsScreen> createState() =>
      _LessonLinkingWordsScreenState();
}

class _LessonLinkingWordsScreenState extends State<LessonLinkingWordsScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_28_Linking_Words/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('linking_words');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_linking_words') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_linking_words', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_linking_words_story_completed') ?? false;
    bool quizDone =
        prefs.getBool('lesson_linking_words_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_linking_words_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_linking_words_quiz_completed', true);
        await prefs.setBool('lesson_linking_words_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_linking_words_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered linking words!\nGreat job!"
