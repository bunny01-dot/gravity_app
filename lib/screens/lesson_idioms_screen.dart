...
// -----------------------------------------------------------------------------
// SCREEN: Common Idioms
// -----------------------------------------------------------------------------
class LessonIdiomsScreen extends StatefulWidget {
  const LessonIdiomsScreen({super.key});

  @override
  State<LessonIdiomsScreen> createState() => _LessonIdiomsScreenState();
}

class _LessonIdiomsScreenState extends State<LessonIdiomsScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_Idioms/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('idioms');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_idioms') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_idioms', true);
  ...
  void _initializeLessonContent() {
    _slides = [
      ...
      LessonQuizInteraction(
        ...
        correctIndex: 1,
        explanation:
            "Correct! 'Butterflies in my stomach' means feeling nervous.",
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone = prefs.getBool('lesson_idioms_story_completed') ?? false;
    bool quizDone = prefs.getBool('lesson_idioms_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_idioms_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_idioms_quiz_completed', true);
        await prefs.setBool('lesson_idioms_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_idioms_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Common Idioms!\nGreat job!"
