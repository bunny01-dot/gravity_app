...
// -----------------------------------------------------------------------------
// SCREEN: Irregular Verbs
// -----------------------------------------------------------------------------
class LessonIrregularVerbsScreen extends StatefulWidget {
  const LessonIrregularVerbsScreen({super.key});

  @override
  State<LessonIrregularVerbsScreen> createState() =>
      _LessonIrregularVerbsScreenState();
}

class _LessonIrregularVerbsScreenState extends State<LessonIrregularVerbsScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_04_Irregular_Verbs/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('irregular_verbs');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_irregular_verbs') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_irregular_verbs', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_irregular_verbs_story_completed') ?? false;
    bool quizDone =
        prefs.getBool('lesson_irregular_verbs_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_irregular_verbs_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_irregular_verbs_quiz_completed', true);
        await prefs.setBool('lesson_irregular_verbs_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_irregular_verbs_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Irregular Verbs!\nGreat job!"
