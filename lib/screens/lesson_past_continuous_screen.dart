...
// -----------------------------------------------------------------------------
// SCREEN: Past Continuous
// -----------------------------------------------------------------------------
class LessonPastContinuousScreen extends StatefulWidget {
  const LessonPastContinuousScreen({super.key});

  @override
  State<LessonPastContinuousScreen> createState() =>
      _LessonPastContinuousScreenState();
}

class _LessonPastContinuousScreenState extends State<LessonPastContinuousScreen>
  ...
  final String _assetPath =
      'assets/Lessons/Lesson_04_Tense_Past/02_Past_Continuous/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('past_continuous');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_past_continuous') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_past_continuous', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_4_past_continuous_story_completed') ?? false;
    bool quizDone =
        prefs.getBool('lesson_4_past_continuous_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_4_past_continuous_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_4_past_continuous_quiz_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_4_past_continuous_score', score);
        }
      }
  ...
  Widget _buildHighlightLayout(LessonHighlightInteraction unit) {
    // Reusing standard slide layout for consistency, or we can make it custom
