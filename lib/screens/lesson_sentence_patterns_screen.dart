...
// -----------------------------------------------------------------------------
// SCREEN: Sentence Patterns
// -----------------------------------------------------------------------------
class LessonSentencePatternsScreen extends StatefulWidget {
  const LessonSentencePatternsScreen({super.key});

  @override
  State<LessonSentencePatternsScreen> createState() =>
      _LessonSentencePatternsScreenState();
}

class _LessonSentencePatternsScreenState
    extends State<LessonSentencePatternsScreen>
  ...
  // Base Asset Path - Assuming a new folder for Lesson 7
  final String _assetPath = 'assets/Lessons/Lesson_07_Sentence_Patterns/';
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_sentence_patterns') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool(
          'has_seen_swipe_hint_lesson_sentence_patterns',
          true,
        );
  ...
  void _initializeLessonContent() {
    _slides = [
      ...
      LessonHighlightInteraction(
        ...
        exampleText:
            "Look at the end of the sentence to find the pattern type!",
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_7_sentence_patterns_story_completed') ?? false;
    bool quizDone =
        prefs.getBool('lesson_7_sentence_patterns_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_7_sentence_patterns_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_7_sentence_patterns_quiz_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_7_sentence_patterns_score', score);
        }
      }
      ...
      if (user != null) {
        ...
        await FirebaseFirestore.instance
            ...
            .doc(user.uid)
            .collection('lessons')
            .doc('lesson_7_sentence_patterns')
