...
// -----------------------------------------------------------------------------
// SCREEN: Question Types
// -----------------------------------------------------------------------------
class LessonQuestionTypesScreen extends StatefulWidget {
  const LessonQuestionTypesScreen({super.key});

  @override
  State<LessonQuestionTypesScreen> createState() =>
      _LessonQuestionTypesScreenState();
}

class _LessonQuestionTypesScreenState extends State<LessonQuestionTypesScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_14_Question_Types/';
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_question_types') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_question_types', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_14_question_types_story_completed') ?? false;
    bool quizDone =
        prefs.getBool('lesson_14_question_types_quiz_completed') ?? false;

    // Legacy fallback
    if (!storyDone) {
      storyDone = prefs.getBool('lesson_14_question_types_completed') ?? false;
    }
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_14_question_types_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_14_question_types_quiz_completed', true);
        await prefs.setBool('lesson_14_question_types_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_14_question_types_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Question Types!\nGreat job!"
