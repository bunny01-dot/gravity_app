...
// SCREEN: Adverbs
// -----------------------------------------------------------------------------
class LessonAdverbsScreen extends StatefulWidget {
  const LessonAdverbsScreen({super.key});

  @override
  State<LessonAdverbsScreen> createState() => _LessonAdverbsScreenState();
}

class _LessonAdverbsScreenState extends State<LessonAdverbsScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_27_Adverbs/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('adverbs');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_adverbs_v4') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_adverbs_v4', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_27_adverbs_story_completed') ?? false;
    bool quizDone = prefs.getBool('lesson_27_adverbs_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_27_adverbs_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_27_adverbs_quiz_completed', true);
        await prefs.setBool('lesson_27_adverbs_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_27_adverbs_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Adverbs!\nGreat job!"
