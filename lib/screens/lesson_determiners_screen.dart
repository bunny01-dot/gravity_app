...
// SCREEN: Determiners
// -----------------------------------------------------------------------------
class LessonDeterminersScreen extends StatefulWidget {
  const LessonDeterminersScreen({super.key});

  @override
  State<LessonDeterminersScreen> createState() =>
      _LessonDeterminersScreenState();
}

class _LessonDeterminersScreenState extends State<LessonDeterminersScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_Determiners/';
  ...
  void initState() {
    ...
    //  Cloud Asset Preload
    LessonContentService().preloadNextLessons('determiners');
  }
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_determiners') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool('has_seen_swipe_hint_lesson_determiners', true);
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_determiners_story_completed') ?? false;
    bool quizDone = prefs.getBool('lesson_determiners_quiz_completed') ?? false;
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_determiners_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_determiners_quiz_completed', true);
        await prefs.setBool('lesson_determiners_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_determiners_score', score);
        }
      }
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              _quizCompleted
                  ? "You have fully mastered Determiners!\nGreat job!"
