...
// -----------------------------------------------------------------------------
// SCREEN: Relative Pronouns
// -----------------------------------------------------------------------------
class LessonRelativePronounScreen extends StatefulWidget {
  const LessonRelativePronounScreen({super.key});

  @override
  State<LessonRelativePronounScreen> createState() =>
      _LessonRelativePronounScreenState();
}

class _LessonRelativePronounScreenState
    extends State<LessonRelativePronounScreen>
  ...
  // Base Asset Path
  final String _assetPath = 'assets/Lessons/Lesson_16_Relative_Pronoun/';
  ...
  Future<void> _checkAndPlayHint() async {
    ...
    final hasSeenHint =
        prefs.getBool('has_seen_swipe_hint_lesson_relative_pronoun') ?? false;
    ...
    if (!hasSeenHint && mounted) {
      ...
      if (mounted && !_hasShownHint) {
        ...
        await prefs.setBool(
          'has_seen_swipe_hint_lesson_relative_pronoun',
          true,
        );
  ...
  Future<void> _loadProgress() async {
    ...
    bool storyDone =
        prefs.getBool('lesson_relative_pronoun_story_completed') ?? false;
    bool quizDone =
        prefs.getBool('lesson_relative_pronoun_quiz_completed') ?? false;

    // Legacy fallback
    if (!storyDone) {
      storyDone = prefs.getBool('lesson_relative_pronoun_completed') ?? false;
    }
  ...
  Future<void> _saveProgress({
    ...
    try {
      ...
      if (storyCompleted) {
        await prefs.setBool('lesson_relative_pronoun_story_completed', true);
      }
      if (quizCompleted) {
        await prefs.setBool('lesson_relative_pronoun_quiz_completed', true);
        await prefs.setBool('lesson_relative_pronoun_completed', true);
        if (score != null) {
          await prefs.setInt('lesson_relative_pronoun_score', score);
        }
      }
