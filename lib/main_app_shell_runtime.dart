...
extension MainAppShellRuntime on _EnglishLearningAppState {
  ...
  void _startResumeAttempt(int runId) {
    _cancelResumeAttemptDeadline();
    // Arm the watchdog now that we know real recovery is needed, not on every resume.
    _armResumeUiWatchdog(source: 'start_resume_attempt');
    _setPhase(_AppPhase.recovering, reason: 'resume_attempt_started');
    _logRecovery(
      runId,
      'resume attempt started timeout_ms=${kMaxResumeWindow.inMilliseconds}',
    );
    _resumeAttemptDeadline = Timer(kMaxResumeWindow, () {
      if (!mounted) return;
      if (_appPhase != _AppPhase.ready) {
        _logRecovery(runId, 'resume timeout -> authoritative safe restart');
        unawaited(_restartToSafeHome(reason: 'resume_timeout'));
      }
    });
  }
  ...
  void _armResumeUiWatchdog({required String source}) {
    _nonRecoveryUiRendered = false;
    _startBlackScreenWatchdog();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted || _isBackgrounded) return;
      _logDiagnostic('resume_ui_frame_observed source=$source');
  ...
  Future<void> _handleAppResumed() async {
    ...
    try {
      ...
      if (resumeTarget == null) {
        _logRecovery(
          runId,
          'pre-check: no active lesson Ã¢â‚¬â€ skipping recovery phase, dashboard stays visible.',
        );
        unawaited(
          _persistSessionSnapshot(pendingRecovery: false, prefs: prefs).timeout(
            _EnglishLearningAppState._resumeIoTimeout,
            onTimeout: () {},
          ),
        );
      ...
      // An active lesson exists Ã¢â‚¬â€ now enter the recovering phase so we can
      // validate and re-push it.
      _startResumeAttempt(runId);

      // Clear pending_recovery immediately on ACCEPT Ã¢â‚¬â€ before any async work.
      // This prevents a re-entry into recovery if the app is interrupted
      // mid-restore (crash, OOM kill, etc.) and resumed again.
      unawaited(
        _persistSessionSnapshot(
          pendingRecovery: false,
          prefs: prefs,
      ...
      bool isAttemptActive() => mounted && _appPhase == _AppPhase.recovering;
      ...
      final storedStage = prefs.getInt(
        _EnglishLearningAppState._currentStageKey,
      );
      final stageValid = storedStage != null && storedStage >= 1;
      ...
      // WARM RESUME Ã¢â‚¬â€ DO NOT NAVIGATE.
      //
      ...
      //   3. Set _appPhase = ready Ã¢â€ â€™ removes BlockingRecoveryOverlay.
      //   4. Mark foregrounded.
      //
      // Cold-start navigation (process was killed) is handled separately by
      // _checkColdStartRecovery Ã¢â€ â€™ _restartToSafeScreen, NOT here.
      // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
      _logRecovery(
        runId,
        'warm resume: active route intact, exiting recovery without nav push',
      );
      if (mounted) {
        setState(() {
      ...
      _recoveryWatchdog?.cancel();
      _blackScreenWatchdog?.cancel();
      _setPhase(_AppPhase.ready, reason: 'warm_resume_no_nav_needed');
      _markReadyUiRendered();
      unawaited(_markForegrounded());
    } catch (e) {
  ...
  Future<void> _initFCM() async {
    ...
    if (isFCMSupported) {
      try {
        ...
        debugPrint("Ã¢Å“â€¦ FCM initialized");
        ...
        debugPrint(
          "Ã¢Å¡Â Ã¯Â¸Â FCM initialization failed (Push Notifications disabled): $e",
        );
      }
    }
  }
