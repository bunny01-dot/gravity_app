...
def replace_method(text: str, name: str, new_block: str) -> str:
    ...
    def is_raw_string_start(prev_char, curr_char):
        return prev_char in ('r', 'R') and curr_char in ('"', "'")
    ...
    while i < len(text):
        ...
        if ch == "'":
            in_single = True
            raw_string = is_raw_string_start(prev_char, ch)
            i += 1
            prev_char = ch
            continue
        if ch == '"':
        ...
        if ch == '(':
            ...
            if not found_body and paren_depth == 0:
                found_body = True
                brace_depth = 1
            elif found_body:
                brace_depth += 1
    ...
    raise ValueError(f'Could not find method body end for {name}')


# Manual blocks with proper indentation
manual_cloud = {
    'syncQuizDataFromCloud': """  Future<void> syncQuizDataFromCloud() async {
    try {
...
""",
    'importCsvFromUrl': """  Future<bool> importCsvFromUrl(String url, String type) async {
    try {
...
""",
    '_mergeAndSaveVocabulary': """  Future<void> _mergeAndSaveVocabulary(
...
""",
    '_mergeAndSaveVerbs': """  Future<void> _mergeAndSaveVerbs(
    List<List<dynamic>> newRows, {
    bool replace = false,
  }) async {
    await _loadVerbData();
...
""",
    '_mergeAndSaveGeneric': """  Future<void> _mergeAndSaveGeneric(
    String type,
    List<List<dynamic>> newRows, {
    bool replace = false,
  }) async {
...
""",
    '_saveDataToPrefs': """  Future<void> _saveDataToPrefs(String key, List<List<dynamic>> data) async {
    final prefs = await SharedPreferences.getInstance();
...
""",
    '_syncWithCloud': """  Future<bool> _syncWithCloud(String key) async {
    try {
...
""",
    '_parseAndCache': """  void _parseAndCache(String key, String jsonString) {
    try {
...
""",
    'saveProgressToCloud': """  /// Saves a specific progress key to Firestore for the current user.
...
""",
    '_updateMasteryTally': """  Future<void> _updateMasteryTally() async {
...
""",
    'getBlackHoleItems': """  Future<List<Map<String, String>>> getBlackHoleItems() async {
    final prefs = await SharedPreferences.getInstance();
...
""",
    'logActivity': """  Future<void> logActivity({
...
manual_admin = {
    '_notifyTeacherOfSuccess': """  Future<void> _notifyTeacherOfSuccess(
    String dateStr,
    int score,
    int total,
  ) async {
...
""",
    'adminSyncFromUrlToCloud': """  Future<bool> adminSyncFromUrlToCloud(String url) async {
    try {
...
""",
    'getUserRole': """  Future<String?> getUserRole() async {
    try {
      final user = FirebaseAuth.instance.currentUser;
...
""",
    'getUserStatus': """  Future<Map<String, dynamic>> getUserStatus() async {
    try {
      final user = FirebaseAuth.instance.currentUser;
...
""",
    'resetStudentProgress': """  Future<void> resetStudentProgress(String uid) async {
    try {
...
""",
    'setUserLevel': """  Future<void> setUserLevel(String level) async {
    final prefs = await SharedPreferences.getInstance();
    final user = FirebaseAuth.instance.currentUser;
...
""",
    'getAssessmentStatus': """  Future<String?> getAssessmentStatus() async {
    final prefs = await SharedPreferences.getInstance();
    final user = FirebaseAuth.instance.currentUser;
    final userId = user?.uid ?? 'guest';
...
""",
    'setAssessmentStatus': """  Future<void> setAssessmentStatus(String status) async {
    final prefs = await SharedPreferences.getInstance();
    final user = FirebaseAuth.instance.currentUser;
    final userId = user?.uid ?? 'guest';
...
""",
    'savePlacementResult': """  Future<void> savePlacementResult(String levelCode, int score) async {
...
# Replace manual cloud methods
cloud_text = cloud_path.read_text(encoding='utf-8')
for name, block in manual_cloud.items():
    cloud_text = replace_method(cloud_text, name, block)
cloud_path.write_text(cloud_text, encoding='utf-8')

# Replace admin methods (entire file rebuild)
admin_header = "// NOTE: Admin/role methods intentionally mix local mutation and remote I/O.\n// Extracted verbatim from DataService; do not refactor or reorder.\npart of 'data_service.dart';\n\nextension DataServiceAdmin on DataService {\n"
admin_methods_order = [
    '_notifyTeacherOfSuccess',
    'adminSyncFromUrlToCloud',
    'getUserRole',
    'getUserStatus',
...
admin_blocks = [manual_admin[name] for name in admin_methods_order]
admin_content = admin_header + '\n'.join(admin_blocks) + "\n}\n"
admin_path.write_text(admin_content, encoding='utf-8')

print('Manual blocks replaced.')
